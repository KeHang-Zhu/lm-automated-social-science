...
class LanguageModel(RegisteredSerializable):
    ...
    def call_openai_api_35(self, prompt: str) -> str:
        #john's first 'org-BBewgcJ4RezFUfkUXpgpKevy'
        # Ben's:  'org-D9XDwg2Sw2y54gEdPpW9Mqvj'
        #same because only charging to John's
        strings = ['org-BBewgcJ4RezFUfkUXpgpKevy', 'org-BBewgcJ4RezFUfkUXpgpKevy']
        weights = [3, 1]  # This means the first string has a 75% chance and the second string has a 25% chance.

        openai.organization = random.choices(strings, weights=weights, k=1)[0]
...
if __name__ == "__main__":
    ...
    if True:
        LLM = LanguageModel(family = 'openai', model = 'gpt-4', temperature = .1)
